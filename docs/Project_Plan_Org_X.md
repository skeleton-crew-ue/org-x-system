...
### Render setup (week 0 — Harsh owns)
...
### Asadbek — QA Engineer (+ Jira bug board owner)
...
**Bug triage (shared with Karim):** brings reproduction steps and severity assessment to the sprint bug-grooming session.
...
To be updated since we starting late
...
- **Schema drift.** All schema changes go through Harsh and Vladimer. Every model change ships as a Django migration that's committed to the repo.
...
- Is the 4,000-record export already available or we generate it?
- Are the 1,000 pages of documents real PDFs, or do we mock them with a representative sample?
- Who owns the Render account, the GitHub org, and the Jira/Confluence workspace? (One person — likely Vladimer — should own all three so access doesn't break.)
